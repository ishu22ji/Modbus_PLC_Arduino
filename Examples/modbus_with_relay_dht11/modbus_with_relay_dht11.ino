...
void setup() {
  ...
  modbus.begin(1, 19200);                                // slave id, baud rate, config (optional)
  Serial.println("Modbus DHT11 Sensor Data");
  delay(200);
  Serial.println("---------------------------------");
}
...
void loop() {
  ...
  holdingRegisters[0]=hum;
  holdingRegisters[1]=tem;
   holdingRegisters[2]=buttonState1;
  holdingRegisters[3]=buttonState2;
  modbus.poll();
  if (buttonState1 == LOW) {
    // turn LED on:
    digitalWrite(11, 1);
